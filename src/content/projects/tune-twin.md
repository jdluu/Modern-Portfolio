...
solution: "Next.js app that reads playlists through youtubei.js and analyzes their characteristics to generate 'twin' recommendations. Material UI frontend, light and dark mode, installable PWA. Search history stays on-device."
process: "Built the YouTube Music core library first, then the vibe-analysis engine on top. Feature-sliced directory structure kept experimentation from turning into spaghetti. Server Actions plus caching handle data fetching, and unit tests cover the analysis logic — the part most likely to break quietly."
impact: "It genuinely surfaces songs I wouldn't have found through the native recommendation loop, filtered by the mood I'm actually in. As a PWA it behaves consistently on desktop and phone."
