...
summary: "Fact-checking assistant: give it a claim, it searches the web and returns an AI verdict with linked sources."
role: "Developer"
technologies:
...
background: "Manually checking claims online means opening ten tabs and reconciling them yourself. I wanted to automate the tedious part: search for evidence, then have a model assess it against the claim — with sources shown so I can disagree with the verdict."
...
process: "I kept search and analysis as separate components — cleaner to test each. The search client has caching and retry logic; the AI can request follow-up searches when initial results aren't sufficient. Most iteration went into result presentation."
impact: "Gives a more grounded answer than a plain search engine because it shows its evidence. Useful both interactively and scripted, thanks to the two interfaces."
